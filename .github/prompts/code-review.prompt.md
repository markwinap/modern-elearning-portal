...
## 5. better-auth / Security

- [ ] Protected pages call `getSession()` (from `~/server/better-auth/server`) and redirect if no session
- [ ] Protected route groups use a session-checking `layout.tsx`
