...
```bash
export OPENWEBUI_API_KEY="<your-open-webui-api-key>"
```

`.opencode/opencode.json` reads this via `{env:OPENWEBUI_API_KEY}`, so the key never lives in the config file.

> **Security**: the API key is stored only in your shell environment or a local `.env` file — never committed to Git. A token was previously committed in `opencode.json`; if you used that key, rotate it in Open WebUI.
